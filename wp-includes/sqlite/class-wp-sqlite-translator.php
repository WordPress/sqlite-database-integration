...
class WP_SQLite_Translator {
	 ...
	 * @var string|null
	 */
	private $last_sqlite_statement = null;
	/**
	 * Most recent SQLite error message.
	 *
	 * @var string|null
	 */
	private $last_sqlite_error = null;
	/**
	 * The query rewriter.
	 *
	...
	public function __construct( $pdo = null ) {
		if ( ! $pdo ) {
			...
			do {
				try {
					$dsn = 'sqlite:' . FQDB;
					$pdo =
						new PDO( $dsn, null, null, array( PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION ) ); // phpcs:ignore WordPress.DB.RestrictedClasses
		...
		// WordPress happends to use no foreign keys.
	...
	private function get_keys( $table_name, $only_unique = false ) {
		...
		foreach ( $indices as $index ) {
			if ( ! $only_unique || '1' === $index['unique'] ) {
				$query     =
					$this->execute_sqlite_query( 'SELECT * FROM pragma_index_info("' . $index['name'] . '") as l;' );
				$results[] = array(
					'index'   => $index,
	...
	private function skip_from_dual( $token ) {
		...
		// FROM DUAL is an Oracle legacy syntax implemented in MySQL that means "no tables".
		$this->rewriter->skip();
		$this->rewriter->skip();
	...
	private function translate_like_escape( $token ) {
		...
		if ( 0 === $this->like_expression_nesting ) {
			...
			/* a keyword, a semicolon, the end of the statement, or a close parenthesis */
			$match = $token->matches( WP_SQLite_Token::TYPE_KEYWORD )
			         || $token->matches( WP_SQLite_Token::TYPE_DELIMITER, null, array( ';' ) )
			         || ( WP_SQLite_Token::TYPE_DELIMITER === $token->type && null === $token->value )
...
// @TODO: Actually rewrite the columns.
	...
	private function execute_delete() {
		 ...
		 */
		$q       =
			$this->execute_sqlite_query( 'SELECT l.name FROM pragma_table_info("' . $table_name . '") as l WHERE l.pk = 1;' );
	...
	public function get_error_message() {
		...
		$output = implode( ' ', array(
				...
				'Original: ',
				htmlspecialchars( $this->mysql_query ),
				'-->',
			) ) . PHP_EOL;
		...
		$output .= '<p>' . htmlspecialchars( $this->mysql_query ) . '</p>' . PHP_EOL;
		$output .= '<p>Error</p>' . PHP_EOL;
		$output .= '<p>' . htmlspecialchars( $msg ) . '</p>' . PHP_EOL;
		$output .= '<p>Queries made or created this session were:</p>' . PHP_EOL;
		$output .= '<ol>' . PHP_EOL;
		foreach ( $this->executed_sqlite_queries as $q ) {
			$message =
				"Executing: {$q['sql']} | " . ( $q['params'] ? 'parameters: ' . implode( ', ', $q['params'] ) : '(no parameters)' );
