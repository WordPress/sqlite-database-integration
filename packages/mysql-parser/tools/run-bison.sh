...
# of the host toolchain, Bison is run inside a pinned Docker image (alpine:3.20
# ships exactly 3.8.2), and the version is ASSERTED so a changed image can never
# silently produce a different automaton. Override with BISON_IMAGE/BISON_VERSION.
#
set -euo pipefail

BISON_IMAGE="${BISON_IMAGE:-alpine:3.20}"
