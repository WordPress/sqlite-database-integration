...
class WP_PostgreSQL_Driver_Tests extends TestCase {
	...
	public function test_insert_ignore_select_without_into_translates_to_postgresql(): void {
		...
		$driver->query( 'CREATE TABLE wptests_insert_ignore_select (id INTEGER PRIMARY KEY, value TEXT NOT NULL)' );
		$driver->query( 'CREATE TABLE wptests_insert_ignore_select_source (id INTEGER NOT NULL, value TEXT NOT NULL)' );
		$driver->query( "INSERT INTO wptests_insert_ignore_select (id, value) VALUES (1, 'existing')" );
		$driver->query( "INSERT INTO wptests_insert_ignore_select_source (id, value) VALUES (1, 'duplicate'), (2, 'new')" );

		$insert = 'INSERT IGNORE wptests_insert_ignore_select (`id`, `value`)
			SELECT id, value FROM wptests_insert_ignore_select_source ORDER BY id';
		...
		$rows = $driver->query( 'SELECT id, value FROM wptests_insert_ignore_select ORDER BY id' );
		$this->assertSame(
			array(
				array( '1', 'existing' ),
				array( '2', 'new' ),
	...
	/**
	 * Tests simple WordPress REPLACE statements delete then insert known conflicts.
	 */
	public function test_simple_wordpress_replace_with_existing_id_is_translated_to_postgresql(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_users" WHERE ("ID" = 2)',
				'INSERT INTO "wptests_users" ("ID", "display_name") VALUES (2, \'Walter Replace Sobchak\')',
			)
		);
	...
	public function test_value_keyword_replace_is_translated_to_postgresql(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_replace_value_keyword" WHERE ("ID" = 2)',
				'INSERT INTO "wptests_replace_value_keyword" ("ID", "display_name") VALUES (2, \'new\')',
			)
		);
	...
	public function test_simple_replace_without_into_is_translated_to_postgresql(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_replace_without_into" WHERE ("ID" = 2)',
				'INSERT INTO "wptests_replace_without_into" ("ID", "display_name") VALUES (2, \'new\')',
			)
		);
	...
	/**
	 * Tests REPLACE ... SET statements delete then insert known conflicts and keep MySQL row counts.
	 */
	...
	/**
	 * Tests multi-row REPLACE statements delete then insert known conflicts and keep MySQL row counts.
	 */
	public function test_multi_row_replace_with_known_conflict_column_is_translated_to_postgresql(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_replace_multi" WHERE ("ID" = 2) OR ("ID" = 3)',
				'INSERT INTO "wptests_replace_multi" ("ID", "display_name") VALUES (2, \'updated\'), (3, \'new\')',
			)
		);
	...
	public function test_replace_uses_metadata_composite_unique_key_conflict_target(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_replace_composite_unique" WHERE ("site_id" = 1 AND "slug" = \'same\') OR ("site_id" = 2 AND "slug" = \'same\')',
				'INSERT INTO "wptests_replace_composite_unique" ("site_id", "slug", "value") VALUES (1, \'same\', \'new\'), (2, \'same\', \'created\')',
			)
		);
	...
	public function test_replace_uses_metadata_prefix_unique_key_conflict_target(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_replace_prefix_unique" WHERE (SUBSTR(CAST("slug" AS text), 1, 10) = SUBSTR(CAST(\'existing-slug-two\' AS text), 1, 10))',
				'INSERT INTO "wptests_replace_prefix_unique" ("slug", "value") VALUES (\'existing-slug-two\', \'new\')',
			)
		);
	...
	public function test_columnless_multi_row_replace_uses_mysql_metadata_columns(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_replace_columnless" WHERE ("ID" = 2) OR ("ID" = 3)',
				'INSERT INTO "wptests_replace_columnless" ("ID", "display_name") VALUES (2, \'updated\'), (3, \'new\')',
			)
		);
	...
	public function test_multi_row_replace_with_duplicate_conflict_values_runs_sequentially(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_replace_duplicate" WHERE "ID" = 4',
				'INSERT INTO "wptests_replace_duplicate" ("ID", "display_name") VALUES (4, \'first\')',
				'DELETE FROM "wptests_replace_duplicate" WHERE "ID" = 4',
				'INSERT INTO "wptests_replace_duplicate" ("ID", "display_name") VALUES (4, \'second\')',
			)
		);
	...
	public function test_simple_wordpress_replace_with_customer_id_is_translated_to_postgresql(): void {
		...
		$this->assert_last_postgresql_sql_statements(
			$driver,
			array(
				'DELETE FROM "wptests_wc_customer_lookup" WHERE ("customer_id" = \'1\')',
				'INSERT INTO "wptests_wc_customer_lookup" ("user_id", "email", "customer_id") VALUES (2, \'new@example.com\', \'1\')',
			)
		);
	...
	public function test_woocommerce_product_lookup_replace_uses_product_id_conflict_and_integer_coercion(): void {
		...
		$queries = $driver->get_last_postgresql_queries();
		$this->assertCount( 2, $queries );
		$this->assertSame( 'DELETE FROM "wptests_wc_product_meta_lookup" WHERE ("product_id" = \'12\')', $queries[0]['sql'] );
		$this->assertStringContainsString( 'INSERT INTO "wptests_wc_product_meta_lookup" ("product_id", "sku", "total_sales") VALUES (\'12\', \'DUMMY SKU100000\', ', $queries[1]['sql'] );
		$this->assertStringContainsString( $this->get_expected_mysql_integer_cast_sql( "'4.000000'" ), $queries[1]['sql'] );
	...
	public function test_non_strict_replace_appends_omitted_not_null_defaults_from_mysql_metadata(): void {
		...
		$expected_statements = array(
			'DELETE FROM "wptests_options" WHERE ("option_name" = \'siteurl\')',
			'INSERT INTO "wptests_options" ("option_name", "option_value", "autoload") VALUES (\'siteurl\', \'\', \'yes\')',
		);

		$this->assertSame( 1, $driver->query( $replace ) );
		$this->assert_last_postgresql_sql_statements( $driver, $expected_statements );
	...
	/**
	 * Tests known-target REPLACE deletes old rows and inserts omitted defaults.
	 */
	public function test_replace_known_unique_conflict_fires_delete_trigger_and_inserts_defaults(): void {
		$driver = $this->create_driver();

		$driver->query(
			'CREATE TABLE wptests_replace_observable (
				id INTEGER PRIMARY KEY,
				slug TEXT NOT NULL UNIQUE,
				value TEXT NOT NULL,
				note TEXT NOT NULL DEFAULT \'default-note\'
		...
		$driver->query( 'CREATE TABLE wptests_replace_delete_log (old_id INTEGER NOT NULL, old_slug TEXT NOT NULL)' );
		$driver->get_connection()->query(
			'CREATE TRIGGER wptests_replace_observable_deleted
				AFTER DELETE ON wptests_replace_observable
				BEGIN
					INSERT INTO wptests_replace_delete_log (old_id, old_slug) VALUES (OLD.id, OLD.slug);
				END'
		);
		$driver->query( "INSERT INTO wptests_replace_observable (id, slug, value, note) VALUES (1, 'same', 'old', 'custom-note')" );

		$replace = "REPLACE INTO wptests_replace_observable (id, slug, value) VALUES (2, 'same', 'new')";
		...
		$rows = $driver->query( 'SELECT id, slug, value, note FROM wptests_replace_observable' );
		$this->assertCount( 1, $rows );
		$this->assertSame( '2', $rows[0]->id );
		$this->assertSame( 'same', $rows[0]->slug );
		$this->assertSame( 'new', $rows[0]->value );
		...
		$log_rows = $driver->query( 'SELECT old_id, old_slug FROM wptests_replace_delete_log' );
		$this->assertCount( 1, $log_rows );
		$this->assertSame( '1', $log_rows[0]->old_id );
		$this->assertSame( 'same', $log_rows[0]->old_slug );
	}
	...
	/**
	 * Tests known-target REPLACE observes ON DELETE CASCADE.
	 */
	public function test_replace_known_unique_conflict_observes_delete_cascade(): void {
		$driver = $this->create_driver();
		$driver->get_connection()->query( 'PRAGMA foreign_keys = ON' );

		$driver->query(
			'CREATE TABLE wptests_replace_cascade_parent (
				id INTEGER PRIMARY KEY,
				slug TEXT NOT NULL UNIQUE,
				value TEXT NOT NULL
			)'
		...
		$driver->query(
			'CREATE TABLE wptests_replace_cascade_child (
				parent_id INTEGER NOT NULL,
				value TEXT NOT NULL,
				FOREIGN KEY (parent_id) REFERENCES wptests_replace_cascade_parent (id) ON DELETE CASCADE
			)'
		);
		$driver->query( "INSERT INTO wptests_replace_cascade_parent (id, slug, value) VALUES (1, 'same', 'old')" );
		$driver->query( "INSERT INTO wptests_replace_cascade_child (parent_id, value) VALUES (1, 'child')" );

		$replace = "REPLACE INTO wptests_replace_cascade_parent (id, slug, value) VALUES (2, 'same', 'new')";
		...
		$parents = $driver->query( 'SELECT id, slug, value FROM wptests_replace_cascade_parent' );
		$this->assertCount( 1, $parents );
		$this->assertSame( '2', $parents[0]->id );
		$this->assertSame( 'same', $parents[0]->slug );
		$this->assertSame( 'new', $parents[0]->value );

		$children = $driver->query( 'SELECT parent_id, value FROM wptests_replace_cascade_child' );
		$this->assertSame( array(), $children );
	}
	...
	public function test_replace_insert_path_with_explicit_identity_repairs_sequence(): void {
		...
		$this->assertSame( 'DELETE FROM "wptests_users" WHERE ("ID" = 2)', $queries[0]['sql'] );
		$this->assertSame( 'INSERT INTO "wptests_users" ("ID", "display_name") VALUES (2, \'Donny Kerabatsos\')', $queries[1]['sql'] );
		$this->assert_sequence_repair_query( $queries[2], 'wptests_users', 'ID', 'wptests_users_ID_seq' );
		$this->assertSame( 1, $connection->get_sequence_sync_query_count() );
	}

	/**
	 * Tests REPLACE conflict replacement paths repair PostgreSQL sequences after insert.
	 */
	public function test_replace_conflict_replacement_path_repairs_sequence(): void {
		$connection = new WP_PostgreSQL_Driver_Alter_Table_Fixture_Connection(
			$this->get_dml_identity_metadata_fixture( 'wptests_users', 'ID', 'wptests_users_ID_seq' )
		);
		$driver     = new WP_PostgreSQL_Driver( $connection, 'wptests' );
		...
		$this->assertCount( 3, $queries );
		$this->assertSame( 'DELETE FROM "wptests_users" WHERE ("ID" = 2)', $queries[0]['sql'] );
		$this->assertSame( 'INSERT INTO "wptests_users" ("ID", "display_name") VALUES (2, \'Walter Replace Sobchak\')', $queries[1]['sql'] );
		$this->assert_sequence_repair_query( $queries[2], 'wptests_users', 'ID', 'wptests_users_ID_seq' );
		$this->assertSame( 1, $connection->get_sequence_sync_query_count() );
	}
	...
	/**
	 * Tests internal MySQL index metadata quotes its PostgreSQL-reserved collation identifier.
	 */
	...
	public function test_upsert_update_assignments_support_values_row_alias_expressions(): void {
		...
		$upsert = 'INSERT INTO `wptests_strict_ints` (`id`, `int_value`)
			VALUES (1, 3) AS incoming
			ON DUPLICATE KEY UPDATE `int_value` = `int_value` + incoming.`int_value`';
		...
		$column_alias_upsert = 'INSERT INTO `wptests_strict_ints` (`id`, `int_value`)
			VALUES (1, 11) AS incoming(row_id, new_value)
			ON DUPLICATE KEY UPDATE `int_value` = new_value';

		$this->assertSame( 1, $driver->query( $column_alias_upsert ) );
		$this->assertSame(
			'INSERT INTO "wptests_strict_ints" ("id", "int_value") VALUES (1, 11) ON CONFLICT ("id") DO UPDATE SET "int_value" = excluded."int_value"',
			$this->get_last_single_postgresql_sql( $driver )
		);
		...
		$this->assertCount( 1, $rows );
		$this->assertSame( '11', $rows[0]->int_value );
	}

	/**
	 * Tests ON DUPLICATE KEY UPDATE preserves no-op self-assignment row counts.
	 */
	public function test_upsert_update_assignments_support_no_op_self_assignments(): void {
		$driver = $this->create_driver();
		$this->install_strict_integer_values_table_with_mysql_metadata( $driver );
		$driver->query( 'INSERT INTO wptests_strict_ints (id, int_value) VALUES (1, 4)' );
		...
		$upsert = 'INSERT INTO `wptests_strict_ints` (`id`, `int_value`)
			VALUES (1, 99)
			ON DUPLICATE KEY UPDATE `int_value` = `int_value`';
	...
	/**
	 * Tests malformed VALUES-row aliases fail closed before reaching PostgreSQL.
	 */
	public function test_upsert_update_assignments_reject_malformed_values_row_aliases(): void {
		$driver = $this->create_driver();
		$this->install_strict_integer_values_table_with_mysql_metadata( $driver );
		...
		foreach (
			...
			try {
				$driver->query( $query );
				$this->fail( 'Expected malformed VALUES-row alias upsert to fail closed.' );
			} catch ( InvalidArgumentException $e ) {
				$this->assertSame( 'Unsupported ON DUPLICATE KEY UPDATE statement.', $e->getMessage(), $query );
				$this->assertSame( array(), $driver->get_last_postgresql_queries(), $query );
			}
	...
	public function test_joined_update_malformed_limit_shapes_fail_closed_before_backend_execution(): void {
		...
		foreach ( $updates as $label => $update ) {
			try {
				$driver->query( $update );
				$this->fail( 'Expected unsupported UPDATE statement to throw for ' . $label . '.' );
			} catch ( InvalidArgumentException $e ) {
				$this->assertSame( 'Unsupported UPDATE statement.', $e->getMessage(), $label );
	...
	public function test_simple_update_order_by_limit_translates_to_ctid_subquery(): void {
		...
		$sql = $this->translate_driver_query_with_private_method(
			$driver,
			'translate_simple_mysql_update_query',
			'UPDATE wptests_update_limited SET `value` = 4 WHERE value > 0 LIMIT 2'
		);

		$this->assertSame(
			'UPDATE "wptests_update_limited" SET "value" = 4 WHERE (ctid IN (SELECT ctid FROM "wptests_update_limited" WHERE value > 0 LIMIT 2)) AND ("value" IS DISTINCT FROM (4))',
			$sql
		);
		...
		$sql = $this->translate_driver_query_with_private_method(
			$driver,
			'translate_simple_mysql_update_query',
			'UPDATE wptests_update_limited SET `value` = 3 LIMIT 1'
		);

		$this->assertSame(
			'UPDATE "wptests_update_limited" SET "value" = 3 WHERE (ctid IN (SELECT ctid FROM "wptests_update_limited" LIMIT 1)) AND ("value" IS DISTINCT FROM (3))',
			$sql
		);
	...
	/**
	 * Tests bounded UPDATE ORDER BY/LIMIT/OFFSET updates the intended ordered slice.
	 */
	public function test_simple_update_order_by_limit_executes_ordered_offset_slice(): void {
		$driver = $this->create_driver();
		...
		$sql = $this->get_last_single_postgresql_sql( $driver );
		$this->assertStringContainsString( 'ctid IN (SELECT "q".ctid FROM "wptests_update_order_limit_exec" AS "q"', $sql );
		$this->assertStringContainsString( 'ORDER BY q.priority ASC, q.id ASC LIMIT 2 OFFSET 1', $sql );

		$rows = $driver->query( 'SELECT id, status FROM wptests_update_order_limit_exec ORDER BY id' );
		$this->assertSame( 'queued', $rows[0]->status );
		$this->assertSame( 'claimed', $rows[1]->status );
		$this->assertSame( 'queued', $rows[2]->status );
		$this->assertSame( 'claimed', $rows[3]->status );
	}
	...
	public function test_mysql_date_and_datediff_runtime_functions_guard_zero_date_casts_for_postgresql(): void {
		...
		$select = "SELECT DATE('0000-00-00 13:05:00') AS zero_date,
			DATE('2020-00-15 13:05:00') AS partial_date,
			DATEDIFF('2024-01-05', '0000-00-00') AS diff_zero,
			DATEDIFF('2020-00-15', '2020-01-15') AS diff_partial";
		$sql    = $this->translate_driver_query_with_private_method(
			$driver,
			'translate_mysql_compatible_query',
			$select
		...
		$this->assertStringContainsString( $this->get_expected_mysql_date_sql( "'0000-00-00 13:05:00'" ) . ' AS zero_date', $sql );
		$this->assertStringContainsString( $this->get_expected_mysql_date_sql( "'2020-00-15 13:05:00'" ) . ' AS partial_date', $sql );
		$this->assertStringContainsString( $this->get_expected_mysql_datediff_sql( "'2024-01-05'", "'0000-00-00'" ) . ' AS diff_zero', $sql );
		$this->assertStringContainsString( $this->get_expected_mysql_datediff_sql( "'2020-00-15'", "'2020-01-15'" ) . ' AS diff_partial', $sql );
		$this->assertStringContainsString( "THEN SUBSTRING(CAST('0000-00-00 13:05:00' AS text) FROM 1 FOR 10)", $sql );
		$this->assertStringContainsString( "THEN SUBSTRING(CAST('2020-00-15 13:05:00' AS text) FROM 1 FOR 10)", $sql );
		$this->assertStringNotContainsString( "TO_CHAR(CAST('0000-00-00 13:05:00' AS timestamp)", $sql );
		$this->assertStringNotContainsString( "CAST('0000-00-00' AS date)", $sql );
		$this->assertStringNotContainsString( "CAST('2020-00-15' AS date)", $sql );
	}
	...
	/**
	 * Tests RAND() and literal RAND(seed) are translated for PostgreSQL.
	 */
	public function test_rand_functions_are_translated_to_postgresql(): void {
		$driver = $this->create_driver();
		...
		$rows = $driver->query(
			"SELECT RAND(0) AS r0,
				...
				RAND(3.9) AS rfloat,
				RAND('5') AS rstring,
				RAND('abc') AS rbadstring"
		);
		$this->assertEqualsWithDelta( 0.15522042769493574, (float) $rows[0]->r0, 1e-12 );
		$this->assertEqualsWithDelta( 0.40540353712197724, (float) $rows[0]->r1, 1e-12 );
		$this->assertEqualsWithDelta( 0.40613597483014313, (float) $rows[0]->r5, 1e-12 );
		$this->assertEqualsWithDelta( 0.15522042769493574, (float) $rows[0]->rnull, 1e-12 );
		$this->assertEqualsWithDelta( 0.15595286540310166, (float) $rows[0]->rfloat, 1e-12 );
	...
	public function test_mysql_date_format_week_specifiers_follow_mysql_week_modes(): void {
		...
		$sql = $this->translate_driver_query_with_private_method(
			$driver,
			'translate_mysql_compatible_query',
			"SELECT DATE_FORMAT('2021-01-03', '%U %u %V %v %X %x') AS formatted_date"
		);
	...
	/**
	 * Tests ALTER TABLE ADD COLUMN accepts parenthesized column lists like the SQLite driver.
	 */
	public function test_alter_table_add_parenthesized_column_list_updates_backend_and_metadata(): void {
		$connection = new WP_PostgreSQL_Driver_Alter_Table_Fixture_Connection();
		$driver     = new WP_PostgreSQL_Driver( $connection, 'wptests' );
		$this->install_information_schema_fixture( $driver );
		$driver->store_mysql_schema_metadata(
			'CREATE TABLE wptests_parenthesized_alter (
				id int(11) NOT NULL,
				PRIMARY KEY (id)
			)'
		);
		...
		$this->assertSame(
			1,
			$driver->query(
				"ALTER TABLE wptests_parenthesized_alter ADD COLUMN (
					title varchar(100) NOT NULL DEFAULT 'draft',
					score int DEFAULT 0
				)"
			)
		);
		...
		$this->assertSame(
			array(
				array(
					'sql'    => 'ALTER TABLE "wptests_parenthesized_alter" ADD COLUMN "title" varchar(100) NOT NULL DEFAULT \'draft\'',
					'params' => array(),
				),
				array(
					'sql'    => 'ALTER TABLE "wptests_parenthesized_alter" ADD COLUMN "score" integer DEFAULT \'0\'',
					'params' => array(),
				),
			),
			$driver->get_last_postgresql_queries()
		...
		$columns = $this->get_mysql_column_metadata_rows( $driver, 'wptests_parenthesized_alter' );
		...
		$create_table = $driver->query( 'SHOW CREATE TABLE wptests_parenthesized_alter' )[0]->{'Create Table'};
		$this->assertStringContainsString( '  `title` varchar(100) NOT NULL DEFAULT \'draft\'', $create_table );
		$this->assertStringContainsString( '  `score` int DEFAULT \'0\'', $create_table );
	}
	...
	public function test_alter_table_add_parenthesized_duplicate_column_fails_before_backend_execution(): void {
		$driver = $this->create_driver();
		$driver->query( 'CREATE TABLE wptests_parenthesized_duplicate_alter (id INTEGER)' );
		$driver->store_mysql_schema_metadata(
		...
		try {
			$driver->query( 'ALTER TABLE wptests_parenthesized_duplicate_alter ADD (slug varchar(20), slug varchar(30))' );
			$this->fail( 'Expected duplicate parenthesized ALTER TABLE ADD COLUMN batch to throw.' );
		} catch ( InvalidArgumentException $e ) {
			$this->assertSame( "Duplicate column name 'slug'.", $e->getMessage() );
			$this->assertSame( array(), $driver->get_last_postgresql_queries() );
		}

		$this->assertSame( array( 'id' ), array_column( $this->get_mysql_column_metadata_rows( $driver, 'wptests_parenthesized_duplicate_alter' ), 'column_name' ) );
	}
	...
	public function test_alter_table_online_ddl_options_are_standalone_noops(): void {
		$queries = array(
			'ALTER TABLE wptests_alter_online_options ALGORITHM=DEFAULT',
			'ALTER TABLE wptests_alter_online_options ALGORITHM COPY',
			'ALTER TABLE wptests_alter_online_options LOCK=NONE',
			'ALTER TABLE wptests_alter_online_options LOCK SHARED',
			'ALTER TABLE wptests_alter_online_options LOCK=EXCLUSIVE',
		);

		foreach ( $queries as $query ) {
			$driver = $this->create_driver();
			$driver->query( 'CREATE TABLE wptests_alter_online_options (id int NOT NULL)' );
			$driver->store_mysql_schema_metadata(
				'CREATE TABLE wptests_alter_online_options (
					id int NOT NULL
				)'
			);
			...
			$this->assertSame( 0, $driver->query( $query ), $query );
			$this->assertSame( array(), $driver->get_last_postgresql_queries(), $query );
			$this->assertSame( array( 'id' ), array_column( $this->get_mysql_column_metadata_rows( $driver, 'wptests_alter_online_options' ), 'column_name' ), $query );
		}
	}

	/**
	 * Tests ALTER TABLE online-DDL options are no-ops inside supported batches.
	 */
	public function test_alter_table_online_ddl_options_are_noops_in_supported_batches(): void {
		$driver = $this->create_driver();
		$driver->query( 'CREATE TABLE wptests_alter_online_batch (id int NOT NULL)' );
		$driver->store_mysql_schema_metadata(
		...
		$driver->query(
			'ALTER TABLE wptests_alter_online_batch
				ALGORITHM=INPLACE,
				ADD COLUMN slug varchar(20) NOT NULL DEFAULT "",
				LOCK=NONE'
		);
		$this->assertSame(
			array(
				array(
					'sql'    => 'ALTER TABLE "wptests_alter_online_batch" ADD COLUMN "slug" varchar(20) NOT NULL DEFAULT \'\'',
					'params' => array(),
				),
			),
			$driver->get_last_postgresql_queries()
		...
		$columns = $this->get_mysql_column_metadata_rows( $driver, 'wptests_alter_online_batch' );
		$this->assertSame( array( 'id', 'slug' ), array_column( $columns, 'column_name' ) );
		$this->assertSame( '', $columns[1]['column_default'] );
	}

	/**
	 * Tests unsupported ALTER TABLE online-DDL option values fail closed.
	 */
	public function test_alter_table_online_ddl_options_fail_closed_for_unsupported_values(): void {
		$queries = array(
			'ALTER TABLE wptests_alter_online_invalid ALGORITHM=INSTANT',
			'ALTER TABLE wptests_alter_online_invalid LOCK=UNKNOWN',
			'ALTER TABLE wptests_alter_online_invalid ADD COLUMN slug varchar(20), ALGORITHM=INSTANT',
		);

		foreach ( $queries as $query ) {
			$driver = $this->create_driver();
			$driver->query( 'CREATE TABLE wptests_alter_online_invalid (id int NOT NULL)' );
			$driver->store_mysql_schema_metadata(
			...
			try {
				$driver->query( $query );
				$this->fail( 'Expected unsupported ALTER TABLE online-DDL option to throw.' );
			} catch ( InvalidArgumentException $e ) {
				$this->assertSame( 'Unsupported ALTER TABLE statement.', $e->getMessage(), $query );
				$this->assertSame( array(), $driver->get_last_postgresql_queries(), $query );
				$this->assertSame( array( 'id' ), array_column( $this->get_mysql_column_metadata_rows( $driver, 'wptests_alter_online_invalid' ), 'column_name' ), $query );
			}
		}
	}

	/**
	 * Tests ALTER TABLE online-DDL options remain unsupported for FULLTEXT/SPATIAL batches.
	 */
	public function test_alter_table_online_ddl_options_exclude_fulltext_and_spatial_batches(): void {
		$queries = array(
			'ALTER TABLE wptests_alter_online_metadata_index ADD FULLTEXT KEY body_fulltext (body), ALGORITHM=INPLACE',
			'ALTER TABLE wptests_alter_online_metadata_index ADD SPATIAL INDEX shape_spatial (shape), LOCK=NONE',
		);
		...
		foreach ( $queries as $query ) {
			...
			try {
				$driver->query( $query );
				$this->fail( 'Expected ALTER TABLE online-DDL option with FULLTEXT/SPATIAL to throw.' );
			} catch ( InvalidArgumentException $e ) {
				$this->assertSame( 'Unsupported ALTER TABLE statement.', $e->getMessage(), $query );
				$this->assertSame( array(), $driver->get_last_postgresql_queries(), $query );
				$this->assertSame( array(), $this->get_mysql_index_metadata_rows( $driver, 'wptests_alter_online_metadata_index' ), $query );
			}
		}
	}
	...
	public function test_show_columns_where_runtime_functions_filter_catalog_rows(): void {
		...
		$result = $driver->query( 'SHOW COLUMNS FROM wptests_options WHERE LENGTH(Field) * 2 = 16' );
	...
	/**
	 * Tests USE information_schema routes final CTE SELECT reads from supported relations.
	 */
	public function test_use_statement_information_schema_cte_final_select_routes_supported_relations(): void {
		$driver = $this->create_driver();
		$this->install_information_schema_fixture( $driver );
		$this->install_direct_information_schema_options_metadata( $driver );
		...
		$rows = $driver->query(
			...
			FROM cols
			JOIN tables AS t ON t.table_name = cols.table_name
		...
		$sql = implode( "\n", array_column( $driver->get_last_postgresql_queries(), 'sql' ) );
		$this->assertStringContainsString( 'WITH "cols" ("table_name") AS', $sql );
		$this->assertStringContainsString( '"cols" AS "cols"', $sql );
		$this->assertStringContainsString( 'AS "t"', $sql );
		$this->assertStringContainsString( 'USING ("TABLE_NAME")', $sql );
	}

	/**
	 * Tests explicit information_schema CTE final SELECT reads route supported relations.
	 */
	public function test_explicit_information_schema_cte_final_select_routes_supported_relations(): void {
		$driver = $this->create_driver();
		$this->install_information_schema_fixture( $driver );
		$this->install_direct_information_schema_options_metadata( $driver );
		...
		$rows = $driver->query(
			...
			FROM cols
			JOIN information_schema.tables AS t ON t.table_name = cols.table_name"
		);
		...
		$this->assertStringContainsString( '"cols" AS "cols"', $sql );
		$this->assertStringContainsString( 'AS "t"', $sql );
		$this->assertStringNotContainsString( 'information_schema.tables AS t', $sql );
	}

	/**
	 * Tests unsupported information_schema CTE SELECT shapes fail closed.
	 */
	public function test_information_schema_cte_selects_fail_closed_for_recursive_and_unsupported_final_sources(): void {
		$queries = array(
			"WITH RECURSIVE cols AS (
				SELECT column_name FROM columns WHERE table_name = 'wptests_options'
			)
			SELECT * FROM cols",
			"WITH cols AS (
				SELECT column_name FROM columns WHERE table_name = 'wptests_options'
			...
			SELECT * FROM information_schema.unsupported_relation",
		);
		...
		foreach ( $queries as $query ) {
			...
			try {
				$driver->query( $query );
				$this->fail( 'Expected unsupported information_schema CTE SELECT to throw.' );
			} catch ( InvalidArgumentException $e ) {
	...
	public function test_direct_information_schema_static_relations_expose_show_backed_metadata(): void {
		...
		$this->assertSame( 'sql_mode', $variables[0]->VARIABLE_NAME );
		$this->assertSame( $driver->get_sql_mode(), $variables[0]->VARIABLE_VALUE );

		$global_variables = $driver->query(
		...
		$this->assertSame(
			...
			array_column( $global_variables, 'VARIABLE_NAME' )
		);
		...
		$processlist = $driver->query( 'SELECT USER, DB, COMMAND, INFO FROM information_schema.processlist' );

		$this->assertCount( 1, $processlist );
		$this->assertSame( 'root', $processlist[0]->USER );
		$this->assertSame( 'wptests', $processlist[0]->DB );
		$this->assertSame( 'Query', $processlist[0]->COMMAND );
		$this->assertStringContainsString( 'information_schema.processlist', $processlist[0]->INFO );
	...
	/**
	 * Tests plugin-facing empty information_schema metadata relations are queryable.
	 */
	public function test_direct_information_schema_empty_plugin_metadata_relations_are_queryable(): void {
		$driver = $this->create_driver();
		...
		$this->assertSame( '0', $views[0]->view_count );

		$trigger_columns = $driver->query( "SHOW COLUMNS FROM information_schema.triggers LIKE 'TRIGGER_%'" );
		$this->assertSame(
			array( 'TRIGGER_CATALOG', 'TRIGGER_SCHEMA', 'TRIGGER_NAME' ),
			array_column( $trigger_columns, 'Field' )
		);

		$routines = $driver->query(
			"SELECT ROUTINE_NAME
	...
	/**
	 * Tests CREATE TABLE ... SELECT without AS and IF NOT EXISTS are translated.
	 */
	public function test_create_table_select_without_as_and_if_not_exists_translates(): void {
		$driver = $this->create_driver();
		$this->install_information_schema_fixture( $driver );

		$driver->query( 'CREATE TABLE ctas_no_as_source (`id` INTEGER, `name` TEXT)' );
		$driver->query( "INSERT INTO ctas_no_as_source (`id`, `name`) VALUES (1, 'one'), (2, 'two')" );

		$this->assertGreaterThanOrEqual(
			0,
			$driver->query( "CREATE TABLE IF NOT EXISTS ctas_no_as SELECT `id` FROM `ctas_no_as_source` WHERE `name` = 'one'" )
		);
		$this->assertSame(
			'CREATE TABLE IF NOT EXISTS "ctas_no_as" AS SELECT "id" FROM "ctas_no_as_source" WHERE "name" = \'one\'',
			$this->get_last_single_postgresql_sql( $driver )
		);

		$rows = $driver->query( 'SELECT * FROM ctas_no_as' );
	...
	public function test_create_table_select_main_database_qualified_target_translates(): void {
		...
		$driver->query( 'CREATE TABLE ctas_qualified_source (id INTEGER, name TEXT)' );
		$driver->query( "INSERT INTO ctas_qualified_source (id, name) VALUES (1, 'one'), (2, 'two')" );

		$this->assertGreaterThanOrEqual(
			0,
			$driver->query( "CREATE TABLE wp.ctas_qualified_copy AS SELECT id FROM ctas_qualified_source WHERE name = 'two'" )
		);
		$this->assertSame(
			'CREATE TABLE "ctas_qualified_copy" AS SELECT id FROM ctas_qualified_source WHERE name = \'two\'',
			$this->get_last_single_postgresql_sql( $driver )
		);

		$rows = $driver->query( 'SELECT * FROM wp.ctas_qualified_copy' );
