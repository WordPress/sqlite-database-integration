...
/**
 * Tests for the WP_MySQL_Parser runtime.
 */
class WP_MySQL_Parser_Tests extends TestCase {
	...
	/**
	 * A parser with unit-production inlining enabled (the collapsed AST).
	 *
	 * @var WP_MySQL_Parser
	 */
	private static $inlining_parser;

	public static function setUpBeforeClass(): void {
		$table                 = require __DIR__ . '/../src/grammar/parse-table.php';
		self::$parser          = new WP_MySQL_Parser( $table );
		self::$inlining_parser = new WP_MySQL_Parser( $table, true );
	}
	...
	public function test_inlining_mode_collapses_unit_production_chains(): void {
		// With unit-production inlining, single-child wrapper rules
		// (expr -> bool_pri -> predicate -> ...) are collapsed: the multi-child
		// bool_pri node for "b = 2" is present, the unit wrappers are not.
		$ast = self::$inlining_parser->parse(
			( new WP_MySQL_Lexer( 'SELECT a + 1 FROM t WHERE b = 2' ) )->remaining_tokens()
		);
		$this->assertNotNull( $ast );
		$this->assertSame( 'sql_statement', $ast->rule_name );   // start_entry is a collapsed unit rule.
		$this->assertNotNull( $ast->get_first_descendant_node( 'bool_pri' ) );
		$this->assertNull( $ast->get_first_descendant_node( 'predicate' ) );
		$this->assertNull( $ast->get_first_descendant_node( 'simple_statement' ) );
	}

	public function test_inlining_mode_accepts_the_same_language(): void {
		$statements = array(
			'SELECT a + 1 FROM t WHERE b = 2' => true,
			'INSERT INTO t (a) VALUES (1)'    => true,
			'SELECT FROM WHERE'               => false,
			'RESET MASTER'                    => false,
		);
		foreach ( $statements as $sql => $accepted ) {
			$tokens = ( new WP_MySQL_Lexer( $sql ) )->remaining_tokens();
			$this->assertSame(
				$accepted,
				null !== self::$inlining_parser->parse( $tokens ),
				"Inlining-mode acceptance differs for: $sql"
			);
		}
	}
